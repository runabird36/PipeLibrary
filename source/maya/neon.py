...
def get_all_connected_nodes(node, down_stream=True, up_stream=False):
    '''
        down stream : nodes from source ( left side )
        up stream   : nodes from target ( right side )
    '''
    # Get all connected nodes to the specified node
    if down_stream == True and up_stream == True:
        connected_nodes = cmds.listConnections(node, d=True, s=True)
    elif down_stream == True and up_stream == False:
        connected_nodes = cmds.listConnections(node, d=False, s=True)
    elif down_stream == False and up_stream == True:
        connected_nodes = cmds.listConnections(node, d=True, s=False)
    ...
    for _node in connected_nodes:
        temp.append(_node)
        temp.extend(get_all_connected_nodes(_node))
    return temp
...
def select_tar_from_RNnode(rn_name :str) -> None:
    query_res = cmds.referenceQuery(rn_name, nodes=True)
    if query_res == []:
        return None
    try:
        cmds.select(query_res[0])
    except:
        return
...
def set_start_frame(_frame :float) -> None:
    cmds.playbackOptions(ast=_frame, min=_frame)
    
def set_end_frame(_frame :float) -> None:
    cmds.playbackOptions(aet=_frame, max=_frame)

def set_start_time(_time :float) -> None:
    cmds.setAttr ("defaultRenderGlobals.startFrame", _time)
    
def set_end_time(_time :float) -> None:
    cmds.setAttr ("defaultRenderGlobals.endFrame", _time)
...
def _get_assetnum_from_assetGRP(connected_geo):
    ...
    # print(111111, abc_file_path)
    # print(len(abc_file_path.split('_')))
    ...
    if not re.search(r"\d{3}", asset_num):
        return ""
...
def is_NS_node(ns_node :str) -> bool:
    return cmds.referenceQuery(ns_node, isNodeReferenced=True)

def is_NS_loaded(ns_node :str) -> bool:
    return cmds.referenceQuery(ns_node, isLoaded=True)
...
def does_MASH_exists() -> bool:
    mash_distributer = cmds.ls(type="MASH_Distribute")
    mash_waiter = cmds.ls(type="MASH_Waiter")
...
def is_connected(from_attr_fullname, to_attr_fullname):
    ...
    if list_cons != None:
        if "|" in from_attr_fullname:
            for con in list_cons:
                print(con)
                if con in from_attr_fullname:
                    return True
        else:
...
def export_ass_v02(export_fullpath, sel_tar, _options_list, frame_info=[], full_path=True, shadow_link=False, light_link=False, get_only_mel=False):
    ...
    # Get mask
    mask = _get_masks_v02(_options_list)
    ...
    assCommand = 'arnoldExportAss -s -boundingBox -exportAllShadingGroups -f \"{0}\" -mask {1}'.format(export_fullpath, mask)
    if full_path == True:
        assCommand += ' -fp'
    if shadow_link == True:
        assCommand += ' -shadowLinks 1'
...
def export_usd(export_fullpath, sel_tar, _options_list, frame_info=[], full_path=True, shadow_link=False, light_link=False, get_only_mel=False):
    ...
    assCommand = 'arnoldExportAss -s -boundingBox -exportAllShadingGroups -f \"{0}\" -mask {1} -fullPath-cam perspShape'.format(export_fullpath, mask)
    if full_path == True:
        assCommand += ' -fp'
    if shadow_link == True:
        assCommand += ' -shadowLinks 1'
